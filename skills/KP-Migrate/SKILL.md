...
description: >
  ...
  wiki/projects/<slug>/, pre-fills pages from the project's existing docs
  and source layout, catalogs credentials and integrations BY REFERENCE
  ONLY (never copies secret values), backs up the existing project
  CLAUDE.md, rewrites it as a thin KP router, and ports any existing ADRs
  / post-mortems / learnings into the vault. Runs on ONE project per
  invocation. Trigger on: "KP-Migrate", "KP-migrate", "migrate this
  project", "migrate existing project", "wire this project to the brain",
  "onboard this project to the vault", "bring this project into the vault",
  "import existing project", "retrofit project to KP".
license: MIT
---
...
There is no fixed secrets folder. This skill catalogs credentials BY REFERENCE ONLY.
When the credentials step runs, ask the owner where this project's secrets live (a
password manager entry, an env file outside git, or a secrets folder they never
commit). Record only the location and the field labels, never the values. If the
owner has no secrets store yet, skip the credentials step and note it in the report.

## When to use

- An existing project that has its own CLAUDE.md but no vault folder.
...
- **Project slug** (kebab-case). Default: derive from folder name (e.g. `My App` becomes `my-app`). Confirm with the owner.
- **Confirmation that the cwd is the project root**, not a subfolder.
- **The secrets location** for this project (see Secrets location above), if it has one.
...
- **Client umbrella check.** If this project lives under a client folder with sibling sub-projects (e.g. one client folder contains multiple deliverables), ask the owner whether this should be migrated as a standalone project or as a sub-project under a client umbrella. If umbrella, the vault path becomes `<vault>/wiki/projects/<client-slug>/<subproject-slug>/` and pages are named `<subproject-slug>-overview.md` etc. The umbrella's own overview lives at `<vault>/wiki/projects/<client-slug>/<client-slug>-overview.md` and links to every sub-project. Create the umbrella page if it does not exist yet.
...
1. Ask the owner where this project's secrets live (see Secrets location above). If they have none, skip to Step 5 and note it.
2. If the owner points you at a secrets file, **read it once, in memory, and extract ONLY the top-level keys (the field/label names)**. Do not log the file content. Do not echo values to chat. Do not write any value into any vault page.
3. For nested objects, extract second-level KEYS only (the field names), never values. Example: from `{"stripe": {"api_key": "x", "webhook_secret": "y"}}` extract the label `stripe` (with sub-fields `api_key`, `webhook_secret`). The actual `"x"` and `"y"` values are never touched.
4. Output a list of integration labels in the vault's `overview.md` Credentials section, in the format defined in Step 6.
5. **If at any point a value looks like it might be a secret (long random string, password, key, token), do NOT include it anywhere.** Treat all values as secret.
...
Will REFERENCE in overview.md Credentials section:
  <secrets location the owner provided>
  Integration labels: stripe, sendgrid, ...
  (Values stay where they are, never copied)
...
2. Write the project pages using KP-Setup's templates, pre-filled with extracted content.
   ...
   - Every file inside `core/` must have `aliases: [<slug>-<type>, ...]` in frontmatter so cross-project wikilinks like `[[my-app-map]]` still resolve.
...
5. Write the new thin router `CLAUDE.md` using KP-Setup's `project-CLAUDE.md` template, filled with `{{PROJECT_NAME}}`, `{{SLUG}}`, `{{STACK}}`, `{{VAULT}}`.
...
7. If a master credentials index exists at `<vault>/wiki/credentials.md`, update it with the project's secrets-location mapping and integration labels (labels only, never values). If no such index exists, skip this step.
...
- Any anomalies (missing secrets file, ambiguous CLAUDE.md content, untouchable source format) that need their attention.
...
_The field list shows WHICH keys exist in the secrets store. Values are never reproduced here._
```
...
- It never reads or copies VALUES from any secrets store. Only top-level and second-level KEY names.
- It never echoes credential values to chat.
- It never writes any secret value into any vault page or any other file.
- It never modifies the contents of the secrets store.
...
- **No secrets store found.** Continue, but flag this in the report. Write the Credentials section with: "No secrets store located. Add the project's secrets location to overview.md manually and list integration labels when ready."
- **Cannot parse the secrets file.** Stop the credentials step, flag it, continue with the rest. Do not guess at the schema.
- **The owner says cancel or dry-run only.** Stop immediately. Delete any partial folder that was created.
- **Source folder is a git submodule.** Stop. Ask the owner if they want to migrate the submodule itself or the parent.
...
1. **Reference, never copy.** Credentials live in the owner's secrets store. The vault refers to them by location and lists labels only.
...
6. **Honesty.** If a step had to be skipped (no secrets store, unparseable file, ambiguous source format), report it plainly. Do not silently fudge.
